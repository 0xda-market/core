...
`ProfitabilityPolicy` consumes this cost before a quote can reserve supply. The automatic-pricing worker described in `automatic-pricing.md` consumes the same cost contract when deriving the minimum solvent client price.
...
5. Quote-time profitability and automatic pricing use the same settlement `CostResult`.
...
5. automatic pricing consuming the same settlement cost contract.

The next adapter may automate a real payment rail, but it must not change marketplace pricing or order lifecycle contracts.
