      ...
      class Service
        ...
        def request(order:, reservation:)
          ...
          decision = Decision.new(
            order_id: order.id, reservation_id: reservation.id,
        ...
        def accept(actor_user_id:, order_id:, expected_version:)
          ...
          decision = @store.transaction do |store|
            ...
            changed = true
            store.replace(
              rebuild(current, status: "accepted", accepted_at: current_time,
        ...
        def complete(actor_user_id:, order_id:, expected_version:, reference: nil, data: {})
          ...
          unless task_id
            raise Core::Conflict.new("fulfillment task is unavailable",
                                     code: "fulfillment_not_ready", details: { order_id: order_id.to_s })
          end
          ...
          unless order.status == "succeeded"
            raise Core::Conflict.new("fulfillment did not complete",
          ...
          completed = @store.transaction do |store|
            ...
            store.replace(
              rebuild(latest, status: "completed", completed_at: current_time,
        ...
        def acknowledge_notification(actor_user_id:, order_id:, event:)
          context = @listings.broker_order_context(actor_user_id: actor_user_id, order_id: order_id)
          field = {
            "broker_order_accepted" => :accepted_notified_at,
            "broker_order_completed" => :completed_notified_at
          }.fetch(event.to_s) { raise ArgumentError, "notification event is invalid" }
          decision = @store.transaction do |store|
            current = store.find(order_id) || raise(Core::NotFound.new("broker_order_decision", order_id))
            ensure_seller!(current, context.listing.seller_user_id)
        ...
        def entry(decision, order: nil, context: nil, changed: false)
          ...
          Entry.new(decision: decision, order: order || @kernel.find_order(decision.order_id),
                    reservation: context.reservation, listing: context.listing, changed: changed)
        end
