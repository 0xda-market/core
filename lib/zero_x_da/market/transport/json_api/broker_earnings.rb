      ...
      class JSONAPI
        module BrokerEarningEndpoints
          ...
          def broker_earnings(request)
            earnings = @broker_earnings.list(actor_user_id: request.params.fetch("actor_user_id"))
          ...
          def broker_payout_profile(request)
            profile = @broker_earnings.payout_profile(actor_user_id: request.params.fetch("actor_user_id"))
            json_response(200, { "data" => profile && present_profile(profile) })
          end
          ...
          def save_broker_payout_profile(request)
            ...
            profile = @broker_earnings.save_payout_profile(
              ...
              network: body.fetch("network"),
              destination: body.fetch("destination"),
              minimum_payout_amount: body.fetch("minimum_payout_amount", "0"),
              enabled: body.fetch("enabled", true),
          ...
          def present_earning(earning)
            {
              ...
              "attributes" => {
                ...
                "quantity" => decimal_string(earning.quantity),
                "ask_amount" => decimal_string(earning.ask_amount),
                "ask_currency" => earning.ask_currency,
                "payable_amount" => decimal_string(earning.payable_amount),
                "payable_currency" => earning.payable_currency,
                "status" => earning.state,
                "payout_id" => earning.payout_id,
                "available_at" => timestamp(earning.available_at),
                "paid_at" => timestamp(earning.paid_at),
                "updated_at" => timestamp(earning.updated_at)
              }.compact
            }
          end
          ...
          def present_balance(balance)
            {
              "pending" => decimal_string(balance.pending),
              "available" => decimal_string(balance.available),
              "payout_queued" => decimal_string(balance.payout_queued),
              "paid" => decimal_string(balance.paid),
          ...
          def present_profile(profile)
            {
              "type" => "broker_payout_profile",
              "id" => profile.seller_user_id,
              "attributes" => {
          ...
          def present_payout(payout)
            {
              ...
              "id" => payout.id,
              "attributes" => {
                "amount" => decimal_string(payout.amount),
                "currency" => payout.currency,
                "network" => payout.network,
                "destination" => payout.destination,
                "status" => payout.state,
                "external_reference" => payout.external_reference,
                "created_at" => timestamp(payout.created_at),
                "paid_at" => timestamp(payout.paid_at),
